...
BEGIN
    ...
    (
        ...
        -- New Columns
        vchNombreEn VARCHAR(250) DEFAULT(''),
        nvchDescripcionEn NVARCHAR(MAX) DEFAULT(''),
    ...
    BEGIN TRY
        INSERT INTO #Result (
            iIdTramite, vchNombre, nvchDescripcion, vchNombreEn, nvchDescripcionEn,
            iIdCategoria, bModalidadEnLinea, mCosto, iIdOficina, 
            dtFechaCreacion, dtFechaActualizacion, bActivo
        )
        SELECT
            T.iIdTramite, T.vchNombre, T.nvchDescripcion, T.vchNombreEn, T.nvchDescripcionEn,
            T.iIdCategoria, T.bModalidadEnLinea, T.mCosto, T.iIdOficina, 
            T.dtFechaCreacion, T.dtFechaActualizacion, T.bActivo
