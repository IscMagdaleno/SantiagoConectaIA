IF OBJECT_ID('SCIA.spGetEmprendimientosFeed') IS NULL
    EXEC('CREATE PROCEDURE SCIA.spGetEmprendimientosFeed AS SET NOCOUNT ON;');
GO

ALTER PROCEDURE [SCIA].[spGetEmprendimientosFeed]
(
    @iPage INT = 1,
    @iPageSize INT = 10,
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
            SELECT
                F.*,
                /* Aleatorio estable por sesión DENTRO de cada tipo (sin mezclar tipos en el hash) */
                ROW_NUMBER() OVER (
